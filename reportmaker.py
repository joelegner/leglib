...
def formula(context, answervar, eq, units=""):
    ...
    vars = [symb for symb in list(context.keys()) if len(symb) <= 4 and isinstance(context[symb], numbers.Number)]
    latex_vars = []
    ...
    eq = "$" + latex_answervar + " = " + eq0 + " = " + eq + " = %s$ " % fmt.sigdig(context[answervar], 3) + units
#    import pdb; pdb.set_trace()
    return eq
